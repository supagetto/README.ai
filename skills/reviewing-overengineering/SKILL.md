...
Scope: over-engineering and complexity only. Correctness bugs, security holes, performance, and formatting/style-guide preferences are explicitly out of scope. Route them to a normal review pass, not this one. A single smoke test or `assert`-based self-check is the floor, not bloat, never flag it for deletion. Does not apply the fixes, only lists them. "stop reviewing-overengineering" or "normal mode": revert to verbose review style.
