...
3. **Write the questionnaire.** Draft questions aimed at the gap from steps 1–2, following the Document structure below. Save to `~/.workbench/questionnaires` - not the current workspace. Name the file `<timestamp>_<description>.md`, using a `YYYY-MM-DD-HHMM` timestamp (e.g. `2026-09-08-1430`) and a short kebab-case description. Use `_` between fields, `-` within fields. Report the path. Done when the file exists and every item the user named in step 2 is covered by a question.
